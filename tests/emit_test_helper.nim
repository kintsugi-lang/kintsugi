...
proc emitLua*(ast: seq[KtgValue], sourceDir: string = "",
  ...
  ## as a single chunk. Strips the `require('prelude')` line that
  ## emitLuaSplit inserts into source — that line only makes sense when
  ## the two outputs land as separate files. In the combined form the
  ## helper bodies live inline.
  ##
