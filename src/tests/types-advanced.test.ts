...
describe('custom typesets', () => {
  test('typeset creates a union type', () => {
    const ev = new Evaluator();
    ev.evalString('string-or-none!: typeset [string! none!]');
  ...
  test('typeset rejects wrong type', () => {
    const ev = new Evaluator();
    ev.evalString('string-or-none!: typeset [string! none!]');
...
describe('typeset with where clause', () => {
  test('where adds a guard to the type', () => {
    const ev = new Evaluator();
    ev.evalString('positive!: typeset/where [integer!] [it > 0]');
...
describe('type matching in match', () => {
  ...
  test('type match with custom typeset', () => {
    const ev = new Evaluator();
    ev.evalString('number!: typeset [integer! float!]');
